...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const statsData = await apiClient('/dashboard');
        setStats(statsData);
        
        const lbData = await apiClient('/leaderboard');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Total Items" value={stats.totalItems} icon={Activity} color="bg-green-500" />
        <StatCard title="Total Results" value={stats.totalResults} icon={List} color="bg-purple-500" />
        <StatCard 
          title="Leading Team" 
          value={leaderboard.length > 0 ? leaderboard[0].name : '-'} 
          icon={Trophy} 
          color="bg-amber-500" 
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <h2 className="font-semibold text-slate-800">Top 3 Teams</h2>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 flex flex-col justify-center items-center p-8 space-y-4">
